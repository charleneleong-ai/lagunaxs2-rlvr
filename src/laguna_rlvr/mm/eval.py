...
def _whisper_baseline(ds, device: str, repo: str) -> float:
    ...
    p = next(model.parameters())  # match the checkpoint dtype (large-v3 loads fp16), not just device
    scores = []
    for wav, label in ds:
        feats = proc.feature_extractor([wav], sampling_rate=_SAMPLE_RATE, return_tensors="pt")
        gen = model.generate(feats["input_features"].to(device=p.device, dtype=p.dtype), max_new_tokens=64)
